...
import com.turbosokol.iqmafiaapp.features.judge.game.JudgeGameAction
...
import com.turbosokol.iqmafiaapp.screens.AchievementScreenView
...
fun BottomNavBarView(viewModel: ReduxViewModel) {
    ...
    val selectedTab = when (navigationState.currentScreenState) {
        ...
        is JudgeAchievementScreenState -> NavigationTabs.ACHIEVEMENT
        ...
        Scaffold(
                ...
                BottomNavigation(elevation = 0.dp, backgroundColor = MaterialTheme.colors.primary) {
                    ...
                    tabsList.forEach { tab ->
                        val (title, icon, action) = when (tab) {
                            ...
                            NavigationTabs.CARDS -> Triple("Cards", Icons.Outlined.Badge, NavigationAction.JudgeCardsScreen(state.getJudgeCardsState()))
                            NavigationTabs.ACHIEVEMENT -> Triple("Achievement", Icons.Outlined.WorkspacePremium, NavigationAction.JudgeAchievementScreen(state.getJudgeAchievementScreenState()))
            ...
            Box(modifier = Modifier.padding(paddingValues)) {
                when (selectedTab) {
                    ...
                    NavigationTabs.ACHIEVEMENT -> AchievementScreenView(viewModel)
